...
CV_joined$CV_filter <- CV_joined$CV <= 0.1
...
CV_joined$y_errorbar[CV_joined$CV_filter == FALSE] <- 
  CV_joined$y_errorbar[CV_joined$CV_filter == FALSE] +
  CV_joined$y_errorbar[CV_joined$CV_filter == TRUE]
...
temp1$Precursors <- "Total precursors"
temp2$Precursors <- "Points across a peak >= 6"
...
temp$Precursors <- factor(temp$Precursors, 
                         levels = c("Total precursors", "Points across a peak >= 6"))
...
```{r}
wrap_plots(pp_plot,id_pr_plot, nrow = 2)
```
