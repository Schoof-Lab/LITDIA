...
# Figure 1. Comparison between an Orbitrap Mass Analyzer and Linear Ion Trap Mass Analyzer on low-input proteomics
...
The `filter_pep()` function is used for every data-set. Furthermore a "filter" data-set, that has removed all peptides missing in more than one replicate, was created.
...
id_pep <- data.frame(method = rep(c(rep("OT 7.5k", 4), rep("OT 15k", 4), rep("OT 30k", 4),
           ...
           id = c(id_OTDIA_7k_1, id_OTDIA_15k_1, id_OTDIA_30k_1, 
                  ...
                  id_LITDIA_Turbo_100, id_LITDIA_Rapi_d_100, id_LITDIA_Normal_100),
...
The mean and standard deviation is calculated for the replicates of each method. Both are plotted in a line plot using `ggplot()`. The function `geom_errorbar()` was used to compute the length of the errorbars from the mean and the standard deviation. `theme_bw()`, `geom_line()`, `geom_point()`, `scale-color_manual()`, `labs()` and `theme()` have just aesthetic effects.
...
Showing the pearson correlation of identified peptides between DIA-OT-based methods and DIA-LIT-based methods, with different input (1 ng, 5 ng, 10 ng and 100 ng). The HeLa lysate was analyzed in quadruple.

## Join LIT and OT data frames

New data frames are created with LIT and OT quantification data for peptides found in both methods. LIT Turbo is compared with OT 7.5k, Rapid with 15k and Normal with 30k. Only peptides found in at least 3 of the 4 replicates are kept. To complete this data frame information the employed method and the input are included.
...
LIT and OT abundances are plotted against each other in a scatter plot using `ggplot()`. The functions `stat_cor()` and `geom_smooth()` was used to add a regression line to the plot. Further a logarithm scale was used for this plot.
...
A new column is added to each LIT data-set to identify peptides which were also found in their corresponding OT data-sets. Information about the scanning mode and input are added.
...
The data frames are combined with the function `rbind()` and arranged with the function `factor()`. Only the columns, which are necessary to figure out, if LIT abundance was found are kept.
...
LIT abundances for peptides found in LIT only (grey) or in both LIT and OT (red) are plotted in a histogram using ggplot. `data_labels1` and `data_labels2` figure out whether LIT abundance was found in OT or not. Further a logarithm scale was used for this plot.
...
Plot 1A, 1B, 1C and 1D are combined using patchwork to create figure 1 and save it as pdf.
